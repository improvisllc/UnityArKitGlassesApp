...
namespace AsPerSpec {
	...
	public class CarouselToggler : MonoBehaviour, IInitializePotentialDragHandler, IEndDragHandler {
        ...
        public bool neighborsOnly = false; // only pick from neighbors in hierarchy
        public delegate void SnapStartedEventHandler(Toggle towards);
        public delegate void SnapMovedEventHandler(Toggle towards);
        public delegate void SnapEndedEventHandler(Toggle towards);

        public event SnapStartedEventHandler OnSnapStarted;
        public event SnapMovedEventHandler OnSnapMoved;
        public event SnapEndedEventHandler OnSnapEnded;
		...
		public void SnapToClosest () {
			...
			if (toggles.Length > 0) {
                ...
                if (OnSnapStarted != null) {
                    OnSnapStarted(toggleClosest);
                }
		...
		Toggle findClosestToggle(Toggle[] toggles) { // closest to scrollRect center
            if (neighborsOnly && toggles.Length>3) { // replace list by only previous,current and next
                int lastIndex = 0;
                bool found = false;
                ...
                if (found) {
                    int newLength = 1;
                    int newLastIndex = 0;
                    bool wrap = horizontalWrap || verticalWrap;
                    if (lastIndex>0 || wrap) { newLength++; newLastIndex++; }
                    if (lastIndex<toggles.Length-1 || wrap) { newLength++; }
                    Toggle[] neighbors = new Toggle[newLength];
                    neighbors[newLastIndex] = toggles[lastIndex];
                    if (lastIndex > 0) { // there's a previous toggle
                        neighbors[newLastIndex - 1] = toggles[lastIndex - 1];
                    }
                    else {
                        if (horizontalWrap || verticalWrap) { // there's no prev toggle, but we're wrapping
                            neighbors[newLastIndex - 1] = toggles[toggles.Length - 1]; //at start, length > 3
                        }
                    }
                    if (lastIndex < toggles.Length - 1) { // there's a next toggle
                        neighbors[newLastIndex + 1] = toggles[lastIndex + 1];
                    }
                    else {
                        if (horizontalWrap || verticalWrap) { // there's no next toggle, but we're wrapping
                            neighbors[newLastIndex + 1] = toggles[0]; // at the end and length > 3, so 0 is ok
                        }
                    }
                    toggles = neighbors; // SWAP!
                }
