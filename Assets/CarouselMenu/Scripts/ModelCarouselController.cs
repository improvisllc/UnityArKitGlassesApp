...
[System.Serializable]
public class ModelCarouselEvent : UnityEvent<int, int> { }

public class ModelCarouselController : MonoBehaviour
{
  public ModelCarouselEvent _onCellClickedEvent;
  ...
  public void AddCell(string name, Texture2D tex)
  {
    ...
    AddCellData(go, m_cellContainer.Count);
        ...
        clicked.callback.AddListener((data) => { GameObject.Find("_manager").GetComponent<UIController>().onModelButtonClicked((PointerEventData)data); });
  ...
  void SetupCells(bool instantUpdate)
  {
    for (int i = 0; i < m_cellContainer.Count; i++)
    {
      ...
      else
      {
                ...
                /*
                print("Garik RT POS: " + rt.position);
                print("Garik FINAL POS: " + m_final);
                if(Vector3.Distance(rt.position, m_final)<0.001)
                {
                    print("Garik::::: FINISH !!!!");
                }
                */
